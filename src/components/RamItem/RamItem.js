...
class RamItem extends Component {
  ...
  handleAdd = (one) => {
    ...
    if(this.state.id0){
      Swal.fire({
      title: 'Are you sure?',
      text: `Adding ${one.props.ram.ram_title}`,
      icon: 'warning',
    ...
    return (
      <>
        <Card class="grey card" className="ui centered card" className="ui fluid card" onClick={this.handleClick}>
          <Image src={this.props.ram.ram_image} width="300px" height="100px" wrapped ui={false}/>
          <Card.Content>
            <Card.Header>{this.props.ram.ram_title}</Card.Header>
            <Card.Description>
              {this.props.ram.ram_description}
            </Card.Description>
          </Card.Content>
          <Card.Content extra>
...
export default withRouter(connect(putReduxStateOnProps)(RamItem));